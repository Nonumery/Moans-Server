...
ACCESS_EXC = HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="No access")
EMAIL_EXC = HTTPException(status_code=status.HTTP_306_RESERVED, detail="Email is already used")
PASS_EXC = HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="Wrong password")